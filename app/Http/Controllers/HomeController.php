...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        /**$barangKeluar = BarangKeluar::selectRaw('YEAR(tanggal_keluar) as tahun, SUM(jumlah) as total')
        ->groupBy('keterangan')
        ->orderBy('tahun')
        ->get();    
       

        $charts['barang_keluar_each_year'] = [
            'series' => [
                [
                    'name' => 'Barang Keluar',
                    'data' => $barangKeluar->pluck('total')->toArray(),
                ]
            ],
            'categories' => $barangKeluar->pluck('tahun')->toArray(),
        ];
         **/   
        $charts = [
